...
本项目在开发过程中遇到的设计权衡和容易踩的坑。这些记录的价值
往往高于完美代码本身--这些地方最容易出错，也最能反映真实工程判断。
...
**原因**：
- shared memory 预算有限
- 可读性优先：：3×3、5×5、7×7 已覆盖常见用例
- 超过 7×7 应改用可分离卷积（O(n²k²) -> O(n²k)）

**收获**：CUDA kernel 不能直接接受动态大小的数组参数。传递可变
大小数据到 kernel 需要 `cudaMemcpy` 到 device 内存或使用动态 shared
memory。这里用固定大小结构体是最简单直接的方案。
...
**收获**：C++ 中 `int` 到 `size_t` 的隐式转换是经典的坑。CUDA
...
**收获**：CI 环境通常没有 GPU。将 CPU 可验证的部分单独分离出来
